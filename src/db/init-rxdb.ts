import {
  addRxPlugin,
  createRxDatabase,
  RxDatabase,
  RxCollection,
  RxJsonSchema,
  RxDocument,
} from "rxdb";

// TUTORIAL
type HeroDocType = {
  passportId: string;
  firstName: string;
  lastName: string;
  age?: number; // optional
};
type HeroDocMethods = {
  scream: (v: string) => string;
};
type HeroDocument = RxDocument<HeroDocType, HeroDocMethods>;

// we declare one static ORM-method for the collection
type HeroCollectionMethods = {
  countAllDocuments: () => Promise<number>;
};

// and then merge all our types
type HeroCollection = RxCollection<
  HeroDocType,
  HeroDocMethods,
  HeroCollectionMethods
>;

type MyDatabaseCollections = {
  heroes: HeroCollection;
};

type MyDatabase = RxDatabase<MyDatabaseCollections>;

import * as paidb from "pouchdb-adapter-idb";
addRxPlugin(paidb);

export async function initRxDb() {
  // addRxPlugin(paidb);

  /**
   * create database and collections
   */
  const myDatabase: MyDatabase = await createRxDatabase<MyDatabaseCollections>({
    name: "mydb",
    // adapter: "indexeddb", // the name of your adapter
    adapter: "idb", // the name of your adapter
  });

  const heroSchema: RxJsonSchema<HeroDocType> = {
    title: "human schema",
    description: "describes a human being",
    version: 0,
    keyCompression: true,
    type: "object",
    properties: {
      passportId: {
        type: "string",
        primary: true,
      },
      firstName: {
        type: "string",
      },
      lastName: {
        type: "string",
      },
      age: {
        type: "integer",
      },
    },
    required: ["firstName", "lastName"],
  };

  const heroDocMethods: HeroDocMethods = {
    scream: function (this: HeroDocument, what: string) {
      return this.firstName + " screams: " + what.toUpperCase();
    },
  };

  const heroCollectionMethods: HeroCollectionMethods = {
    countAllDocuments: async function (this: HeroCollection) {
      const allDocs = await this.find().exec();
      return allDocs.length;
    },
  };

  await myDatabase.addCollections({
    heroes: {
      schema: heroSchema,
      methods: heroDocMethods,
      statics: heroCollectionMethods,
    },
  });

  // add a postInsert-hook
  // myDatabase.heroes.postInsert(
  //   function myPostInsertHook(
  //     this: HeroCollection, // own collection is bound to the scope
  //     docData: HeroDocType, // documents data
  //     doc: HeroDocument // RxDocument
  //   ) {
  //     console.log("TCL: initRxDb -> docData", docData);
  //     console.log("insert to " + this.name + "-collection: " + doc.firstName);
  //   },
  //   false // not async
  // );

  /**
   * use the database
   */

  // insert a document
  // const hero: HeroDocument = await myDatabase.heroes.insert({
  //   passportId: "myId",
  //   firstName: "piotr",
  //   lastName: "potter",
  //   age: 5,
  // });
  myDatabase.heroes;
  console.log("TCL: initRxDb -> myDatabase.heroes", myDatabase.heroes);

  // access a property
  // console.log(hero.firstName);

  // // use a orm method
  // // hero.scream("AAH!");
  // // hero.atomicUpdate((oldData) => {
  // //   console.log("TCL: initRxDb -> oldData", oldData);
  // //   return oldData;
  // // });

  // // use a static orm method from the collection
  // const amount: number = await myDatabase.heroes.countAllDocuments();
  // console.log(amount);
  window["myDatabase"] = myDatabase;

  await myDatabase.destroy();
}

// https://rxdb.info/adapters.html#indexeddb-new
// addRxPlugin(paidb);

// export async function initRxDb() {
//   // const database = await createRxDatabase({
//   //   name: "mydatabase",
//   // });

//   const db = await createRxDatabase({
//     name: "heroesdb",
//     adapter: "idb", // the name of your adapter
//     // adapter: 'indexeddb',
//     password: "myLongAndStupidPassword", // optional
//   }); // create database

//   await db.collection({ name: "heroes", schema: mySchema }); // create collection
//   db.heroes.insert({ name: "Bob" }); // insert document
// }
